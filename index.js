...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooAnimals){
    ...
    zooAnimals.forEach(animal =>{
      const name = `name: ${animal.animal_name}`;
      const scientific = `scientific: ${animal.scientific_name}`;
      const displayName = `${name}, ${scientific}`;
      displayNames.push(displayName);
    });
    return displayNames;
  }
 
 const displayNames = animalNames(zooAnimals);
  ...
  */
  ...
  const lowPopulation = lowPopulationAnimals(zooAnimals);
  ...
  */
  ...
  const totalPpulation = USApop(zooAnimals);
  console.log("Total population across United States " + totalPpulation);
...
const cuboidTwo=new
CuboidMakerTwo({length:5, width:6, height:7});
